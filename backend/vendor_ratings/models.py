...
class VendorRating(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(["appointment_id"], ["appointments.appointment_id"], ondelete="SET NULL", onupdate="CASCADE", name="vendor_ratings_appointments_appointment_id_fk"),
        Index("vendor_ratings_students_student_id_fk", "student_id"),
        Index("vendor_ratings_vendors_vendor_id_fk", "vendor_id"),
        Index("vendor_ratings_appointments_appointment_id_fk", "appointment_id", unique=True),
    )
    ...
    appointment: Mapped[Optional["Appointment"]] = relationship("Appointment", back_populates="vendor_ratings")
