...
from appointments.schemas import AppointmentUpdate
...
def main() -> int:
    ...
    try:
        with Session(engine) as db:
            ...
            appointments = AppointmentRepository(db).get_all()
        ...
        checks = [
            ...
            ("Service", ServiceRepository, services, lambda x: ServiceUpdate(price=decimal.Decimal(x.price) + 1)),
            ("Item", ItemRepository, items, lambda x: ItemUpdate(stock=x.stock + 1)),
            ("TimeBlock", TimeBlockRepository, time_blocks, lambda x: TimeBlockUpdate(day_of_week=(x.day_of_week + 1) % 7)),
            ("Appointment", AppointmentRepository, appointments, lambda x: AppointmentUpdate(status="no-show")),
